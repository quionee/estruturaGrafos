		...
		for h in range(tamLista):
			...
			if self.tipoEstrutura == "A":
				...
				if self.tipo == "D":
					i = 0
					encontrou = False
					while i < len(estrutura[u]) and not(encontrou):
						if estrutura[u][i] == v2:
							print "ENCONTREI, VOU EXCLUIR"
							del estrutura[u][i]
							encontrou = True
							self.qtdArestas -= 1
						i += 1
					if len(estrutura[u]) == 0:
						print "IGUAL A ZERO O TAMANHO"
						j = 0
						existe = False
						while j < self.qtdVertices and not(existe):
							...
							while k < len(estrutura[j]) and not(existe):
								if estrutura[j][k] == u2:
									print "ENCONTREI OUTRO, NAO GASTA EXCLUIR"
									existe = True
								k += 1
							j += 1
						if not(existe):
							"EXCLUINDO, POIS NAO EXISTE MAIS NINGUEM IGUAL A MIM"
							del estrutura[u]
							self.qtdVertices -= 1
